...
program define litdiscover, rclass
    ...
    */
    ...
    /* -----------------------------------------------------------------
       ...
       pyldavispath is set only when the interactive flag is supplied;
       litdiscover.py writes the pyLDAvis-ready interchange file there.
       Empty otherwise, in which case litdiscover.py emits nothing extra.
       -----------------------------------------------------------------
    */
    ...
    */
    ...
    /* -----------------------------------------------------------------
       ...
       exist only under that condition. The Python helper reads
       litdiscover_cooc_within.dta and litdiscover_cooc_cross.dta
       directly, writes two CSVs, which are then imported as .dta with
       the standard litdiscover_ prefix.
       -----------------------------------------------------------------
    */
    ...
    /* -----------------------------------------------------------------
       Step 5a (Block A.5): Stata-tier figures

       Each figure: emit .gph and .png. Resolution 300 DPI via the
       graph export width parameter (Stata maps width to DPI at 7.5 inches
       effective width; width=2250 yields 300 DPI). Palette: viridis for
       sequential heatmaps, tableau for categorical bars.
       -----------------------------------------------------------------
    */
    local figures_n = 0
    local figures_list ""
    ...
    /* -----------------------------------------------------------------
       ...
       locals (figures_list_py, interactive_list_py).
       -----------------------------------------------------------------
    */
